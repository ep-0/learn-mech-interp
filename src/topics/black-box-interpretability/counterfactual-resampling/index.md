...
exitCriteria:
  - task: "Explain why the forced-answer baseline measures the wrong thing, and how counterfactual resampling changes the question being asked."
    answer: |
      **Forced-answer** interrupts the model after step $i$ and makes it answer. It measures **cumulative information**: how much has accumulated by this point. Accuracy jumping after a step is then read as that step being important.

      The flaw is that a step can coincide with an accuracy jump merely by *following* the real work. If steps 3 and 4 did the computation and step 5 restates the result, forced-answer credits step 5, because that is where the answer first becomes readable. It conflates *when the answer becomes available* with *what made it possible*.

      **Resampling changes the question** from "does the model know the answer yet?" to "would the answer change if this step had gone differently?" Delete $S_i$ and everything after it, regenerate a replacement $T_i$ from the same prefix using the model's own sampling, roll out to a final answer, repeat 100 times, and compare the answer distributions with and without the original step.

      That is a counterfactual rather than a cutoff. A step that merely restates prior work is replaced by another restatement and the answer distribution does not move; a step that did the work is replaced by a different attempt and the distribution shifts.
  - task: "The replacement sentence is regenerated by the model itself at temperature 0.6, rather than deleted or replaced with a neutral filler. Explain why, using the ablation-baseline problem."
    answer: |
      It is the resampling-ablation choice applied to text, and for the same reason: **a baseline is a substituted value, and every value means something to the model.**

      Deleting the sentence outright produces a reasoning trace with a hole — a prefix that jumps discontinuously. That is off-distribution in a specific way, and the model's response to it may reflect confusion at the discontinuity rather than the absence of that step's content. Substituting neutral filler is worse, since it inserts text no reasoning trace would contain.

      Regenerating from the same prefix, with the model's own sampling, produces a continuation the model would plausibly have produced. The counterfactual is then "this reasoning went differently" rather than "this reasoning was corrupted," which is the question worth asking.

      Temperature $0.6$ matters too. At temperature $0$ the regeneration would reproduce $S_i$ almost exactly and measure nothing; too high and the replacements are unrepresentative. The setting has to produce genuine alternatives that the model would actually generate.

      The 100 rollouts are the other half: a single resample gives one draw from a distribution, and the quantity of interest is a divergence between distributions.
  - task: "Counterfactual resampling uses only inputs and outputs. Say what this buys relative to activation patching and what it cannot deliver."
    answer: |
      **What it buys:**

      - **Black-box applicability.** It needs only sampling access, so it works on models whose internals are unavailable — the setting where most deployed systems live, and where activation patching is simply unavailable.
      - **The right unit for the question.** Reasoning steps are semantic objects spanning many tokens and positions. Patching operates on components at positions and has no natural handle on "the sentence where the model caught its error."
      - **Genuine counterfactuals in the model's own distribution**, since replacements are generated rather than constructed.

      **What it cannot deliver:** any account of *mechanism*. It establishes that a sentence's content changes the answer distribution — a causal claim at the level of the text — and says nothing about which heads, features, or layers implement the dependency. It cannot distinguish a step that matters because the model reads its content from one that matters because it changes the format or length of what follows.

      So the two methods sit at different levels and answer different questions. The natural combination is to use resampling to find which steps are load-bearing, then patching to ask how those steps are read — which is what the receiver-head and attention-suppression analyses in the same framework begin to do.
